...
const Card = ({ e, onOpen }) => (
  <button data-testid={`profile-card-${e.id}`} onClick={() => onOpen(e.id)}
    className="glass fade-up flex items-center gap-3 rounded-2xl p-3 text-left transition-transform active:scale-[0.98]">
...
export const Profili = ({ onOpenProfile }) => {
    ...
    <div className="fade-up">
      ...
      <div className="mb-3 flex gap-2">
        {[["player", "Giocatori"], ["coach", "Allenatori"]].map(([k, l]) => (
          <button key={k} data-testid={`kind-${k}`} onClick={() => setKind(k)}
            className={`rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition-all ${kind === k ? "bg-[#24E07A] text-black" : "glass text-white/60"}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div className="mb-4 flex gap-2">
        ...
        <select data-testid="profili-team-filter" value={team} onChange={(e) => setTeam(e.target.value)}
          className="rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white focus:border-[#24E07A] focus:outline-none">
      ...
      {list.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {list.map((e) => <Card key={e.id} e={e} onOpen={onOpenProfile} />)}
        </div>
      )}
    </div>
  );
