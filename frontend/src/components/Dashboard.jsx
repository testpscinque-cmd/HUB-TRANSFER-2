...
import { Search, Zap, X, ExternalLink, Play, Newspaper } from "lucide-react";
...
const NewsCard = ({ n }) => (
  <a href={n.link} target="_blank" rel="noopener noreferrer" data-testid={`post-${n.id}`}
    className="glass fade-up flex flex-col gap-2 rounded-2xl p-4 transition-transform active:scale-[0.99]">
    ...
    <div className="flex items-center justify-between">
      ...
      <span className="flex items-center gap-1 text-[11px] font-bold text-green"><ExternalLink size={12} /> Vai a</span>
    </div>
  </a>
);

const VideoCard = ({ v, onOpen, onOpenProfile }) => (
  <div data-testid={`video-${v.id}`} className="glass fade-up overflow-hidden rounded-2xl transition-transform active:scale-[0.99]">
    ...
    <div className="flex flex-col gap-2 p-4">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onOpenProfile(slug(v.player))} className="text-[11px] font-bold text-green hover:underline">{v.player}</button>
        <span className="text-white/30">·</span>
        <span className="flex items-center gap-1 text-[11px] text-white/70">{v.channel} {v.verified && <VerifiedTick size={12} />}</span>
        <span className="ml-auto"><StatusBar color={v.color} /></span>
      </div>
    </div>
  </div>
);
...
export const Dashboard = ({ onOpenProfile }) => {
  ...
  return (
    <div className="fade-up">
      {/* Sticky Matchmaker */}
      <div className="sticky top-0 z-30 -mx-4 mb-4 px-4 pt-1 pb-3" style={{ background: "linear-gradient(#07070B ,#07070Bee 70%, transparent)" }}>
        <div className="mb-3 flex items-center gap-2">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#24E07A] glow-green"><Zap size={19} className="text-black" fill="black" /></span>
          <h1 className="font-heading text-2xl font-black uppercase tracking-tight text-white">Transfer<span className="text-green">Hub</span></h1>
        ...
        <div className="glass rounded-2xl p-3">
          <div className="grid grid-cols-2 gap-2">
            <input list="player-list" data-testid="mm-player" value={pName} onChange={(e) => setPName(e.target.value)} placeholder="Giocatore / Allenatore"
              className="rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white placeholder:text-white/35 focus:border-[#24E07A] focus:outline-none" />
          ...
          <button data-testid="match-scoop-btn" onClick={runMatch} disabled={matching}
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-[#24E07A] py-2.5 font-heading text-sm font-black uppercase tracking-wider text-black transition-all active:scale-[0.98] disabled:opacity-60">
            {matching ? <span className="h-4 w-4 rounded-full border-2 border-black/40 border-t-black spin" /> : <Zap size={16} fill="black" />}
      ...
      <div className="mb-3 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input data-testid="news-search-input" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && runSearch()}
            ...
            className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-9 pr-3 text-sm text-white placeholder:text-white/35 focus:border-[#24E07A] focus:outline-none" />
        </div>
      ...
      <div className="mb-4 flex gap-2">
        {[["all", "Tutto"], ["posts", "Post"], ["videos", "Video"]].map(([k, l]) => (
          <button key={k} data-testid={`filter-${k}`} onClick={() => setFilter(k)}
            className={`rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition-all ${filter === k ? "bg-[#24E07A] text-black" : "glass text-white/60"}`}>
            {l}
          </button>
        ))}
      ...
      {loading ? (
        <div className="flex justify-center py-16"><span className="h-8 w-8 rounded-full border-2 border-white/20 border-t-[#24E07A] spin" /></div>
      ) : (
        <div className="space-y-3">
          ...
          {feed.map((item) => item.type === "video"
            ? <VideoCard key={item.id} v={item} onOpen={setVideo} onOpenProfile={onOpenProfile} />
